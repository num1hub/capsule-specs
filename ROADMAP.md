...
- machine-readable governance, boundary, failure, and publication-readiness models are transitioning to capsule-backed projection on the public `dream` branch while remaining human-readable public JSON outputs in the tracked repo
...
- improve onboarding for external contributors and integrators
  ...
  Deepened further through direct repo-relative, installed-package, and extracted-artifact Python OpenAPI consumer recipes, so integrators no longer have to jump from route docs straight into the raw OpenAPI file without copyable strongest-surface reading paths.
  Deepened further through [`docs/openapi-codegen-recipes.md`](docs/openapi-codegen-recipes.md), [`examples/client/openapi-generate-validator-types.mjs`](examples/client/openapi-generate-validator-types.mjs), [`examples/client/cjs-package-openapi-codegen.cjs`](examples/client/cjs-package-openapi-codegen.cjs), [`examples/client/esm-package-openapi-codegen.mjs`](examples/client/esm-package-openapi-codegen.mjs), [`examples/client/ts-package-openapi-codegen.ts`](examples/client/ts-package-openapi-codegen.ts), and [`scripts/check-openapi-codegen.js`](scripts/check-openapi-codegen.js), so tool-builders now have a verifier-backed `openapi-typescript` path from repo-local plus CommonJS, ESM, and TypeScript installed-package surfaces instead of stopping at direct OpenAPI inspection.
  Deepened further through [`scripts/check-openapi-coherence.js`](scripts/check-openapi-coherence.js), [`docs/openapi.md`](docs/openapi.md), and [`docs/reference-pack.md`](docs/reference-pack.md), so the compact validator-route and envelope-family layers now have a dedicated stronger-source guard against drift from the published OpenAPI contract.
