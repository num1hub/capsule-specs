const batchRequest = require("@num1hub/capsule-specs/examples/api/validate-request.batch.json");
const fixRequest = require("@num1hub/capsule-specs/examples/api/validate-request.fix.json");
const singleRequest = require("@num1hub/capsule-specs/examples/api/validate-request.single.json");
const {
  publishedValidatorRouteDefinitions,
  publishedValidatorRoutes
} = require("@num1hub/capsule-specs/typescript/validator-routes");
...
const exampleSingleRequest = singleRequest;
const exampleBatchRequest = batchRequest;
const exampleFixRequest = fixRequest;
...
function createValidatorClient(baseUrl, token) {
  const authHeaders = {
...
if (exampleRouteSummary.batchCapsuleCount !== 2) {
  throw new Error("CommonJS package live-validator batch request drifted from the published sample count");
}

if (!exampleRouteSummary.statsQueryParameters.includes("limit")) {
  throw new Error("CommonJS package live-validator route metadata drifted from the published stats query coverage");
}
