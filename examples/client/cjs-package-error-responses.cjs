...
const statsErrorResponse = require("@num1hub/capsule-specs/examples/api/stats-error-response.sample.json");
...
const parsedResponses = [
  ...
  simpleErrorResponseSchema.parse(statsErrorResponse)
];
...
if (parsedResponses[5].error !== "Stats computation failed") {
  throw new Error("missing stats package error response");
}
