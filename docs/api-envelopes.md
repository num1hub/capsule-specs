...
The published validator API projections strengthen that path further by giving TypeScript and Zod consumers a public-safe source-level envelope layer for `validate`, support, and bounded shared error envelopes while staying subordinate to the stronger JSON Schema and OpenAPI surfaces.
