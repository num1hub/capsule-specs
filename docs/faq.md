...
Start with `README.md`, `docs/reviewer-guide.md`, `PUBLIC_EVALUATION_PACKET.json`, `PUBLIC_TRACEABILITY_MATRIX.json`, `PUBLIC_EXAMPLE_COVERAGE.json`, `PUBLIC_MAINTENANCE_MODEL.json`, `PUBLIC_FAILURE_MODEL.json`, `PUBLIC_PROJECT_PROFILE.json`, `PUBLIC_CAPABILITY_MATRIX.json`, and `PUBLIC_RELEASE_METADATA.json`. Those surfaces explain scope, health signals, supported outcomes, and where the strongest public evidence lives.
